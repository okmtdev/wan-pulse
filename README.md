...
| `wan_pulse/notify.py` | Slack 通知（テキスト／音声アップロード・任意） |
| `wan_pulse/history.py` | 検知履歴を Google スプレッドシート／CSV に蓄積（任意） |
| `wan_pulse/web.py` | ローカル管理アプリ（一覧・再生・ラベル付け・任意） |
| `wan_pulse/cli.py` | `init` / `devices` / `monitor` / `run` / `classify` / `web` の各コマンド |
...
録音した区間を**ブラウザで一覧・再生・ラベル付け**できる、ローカル専用の管理アプリです
（依存ゼロ＝Python 標準ライブラリのみ）。後で本格的にラベル付け（学習データ作り）を
やるための土台。

できること:
- `recordings/` を走査して **wav + サイドカー JSON をペアで一覧**（新しい順）
- その場で**音声再生**、自動判定（犬か・感情・スコア・peak など）を表示
- フィルタ（すべて / 犬のみ / 未レビュー）、上部に件数サマリ
- **レビュー（ラベル付け）**: 「犬？(はい/いいえ/未設定)・感情・メモ」を保存
  → サイドカー JSON の `review` に書き込み（自動判定フィールドは壊さない）

### 起動

```bash
wan-pulse web                       # 単体起動（http://127.0.0.1:8765）
wan-pulse web --web-port 9000       # ポート変更
```

`wan-pulse run` と**同時に**立ち上げる（録音しながらブラウズ）:

```bash
wan-pulse run --web                 # run と一緒に web も起動
```

`wan-pulse.toml` で常時オンにする:

```toml
[web]
enabled = true        # run と同時に起動（run --web と同等）
host = "127.0.0.1"    # ローカルのみ。LAN から見たいなら "0.0.0.0"（後述の注意）
port = 8765
```

ブラウザで `http://127.0.0.1:8765` を開くと一覧が出ます。レビューを保存すると、その区間の
`.json` に次のように追記されます:

```json
"review": { "is_dog": true, "emotion": "警戒・興奮", "note": "宅配が来た時", "reviewed_at": "2026-05-30T22:10:05" }
```

> - 既定は **127.0.0.1（このマシンからのみアクセス可）**。`host = "0.0.0.0"` にすると
>   同じ LAN の他端末からも見えます（認証は無いので、信頼できるネットワークのみで）。
> - 別マシン（Mac）で録音済みデータを見たいだけなら、`recordings/` をコピーして
>   `wan-pulse web --output-dir path/to/recordings` でもOK。
...
- 🟡 **ラベル付けの回し**: ローカル管理アプリ（上記「ウェブ UI」）でレビュー＝土台は完成。
  ここで貯めた `review` を学習データにしていく
